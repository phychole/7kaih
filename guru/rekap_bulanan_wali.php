...
<div class="content">
  <h3><i class="fas fa-calendar-alt"></i> Rekap Bulanan (Siswa Wali)</h3>
      ...
      <form method="GET" class="row g-2 align-items-end">
        ...
        <div class="col-md-7 text-end">
          ...
          <a target="_blank"
             ...
             href="rekap_bulanan_wali_pdf.php?year=<?= $year ?>&month=<?= $month ?>">
  ...
  WHERE gw.id_guru = ?
  GROUP BY s.id, s.nama, s.nisn, s.agama, k.nama_kelas
  ORDER BY k.nama_kelas ASC, s.nama ASC
  ";

  $q = $conn->prepare($sql);
  if(!$q) die("Query gagal: ".$conn->error);
  $q->bind_param("ssi", $start, $end, $id_guru);
  $q->execute();
  $data = $q->get_result();
  ?>
    ...
    <table class="table table-bordered table-striped align-middle">
      ...
      <tbody>
        <?php if($data->num_rows==0): ?>
          <tr><td colspan="14" class="text-center text-danger">Tidak ada data pada periode ini.</td></tr>
        <?php endif; ?>
        ...
        ?>
          <tr class="text-center">
            ...
            <td><span class="badge bg-<?= ($ib_lengkap>0?'success':'warning') ?>"><?= $ib_lengkap ?> / <?= $ib_ada ?></span></td>
            <td><span class="badge bg-<?= ($mk_lengkap>0?'success':'warning') ?>"><?= $mk_lengkap ?> / <?= $mk_ada ?></span></td>
          </tr>
        <?php endwhile; ?>
      </tbody>
    </table>
